...
public:
	XString_Iterator() = default;
	XString_Iterator(char* p) : p{ p } {}

	//반복자끼리 비교 - relational operator
	//<=>를 프로그래밍 해주면 자동으로 ==, !=, <, >, <=, >=가 만들어진다.
	auto operator<=>(const XString_Iterator& rhs) const = default;
	difference_type operator-(const XString_Iterator& rhs) const {
		return p - rhs.p;
	}
	//전위 증가 코드. 때문에 p++라고 하면 안된다.
	...
	XString_Iterator operator+(difference_type n) const {
		return p + n;
	}
	XString_Iterator operator-(difference_type n) const {
		return p - n;
